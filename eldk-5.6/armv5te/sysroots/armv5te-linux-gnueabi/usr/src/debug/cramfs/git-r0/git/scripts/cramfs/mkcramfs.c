...
static const char *mkcramfs_version = "mkcramfs (cramfsprogs) 1.2";
...
static void fix_inode(struct cramfs_inode *inode)
{
	...
	offset = inode->offset;
	CRAMFS_SET_OFFSET(inode, offset);
	CRAMFS_SET_NAMELEN(inode, namelen);
#endif
}
...
static void fix_offset(struct cramfs_inode *inode, u32 offset)
{
	...
	((u32*)inode)[2] = wswap(offset | (tmp&0xfc000000));
#else
	CRAMFS_SET_OFFSET(inode, offset);
#endif
}
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "bD:Ee:hi:ln:pqrsvVz")) != EOF) {
		switch (c) {
		case 'h':
			usage(MKFS_OK);
		case 'V':
